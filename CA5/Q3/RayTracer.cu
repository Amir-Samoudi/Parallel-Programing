...
__device__ Vec3 rayColor(const Ray& r, Hittable* objects, int num_objects, Vec3 light_pos) {
    ...
    if (hit_index >= 0) {
        ...
        for (int i = 0; i < num_objects; ++i) {
            ...
            if (objects[i].type == SPHERE) {
                in_shadow = hitSphere(objects[i], shadow_ray, t_min, t_max, t, temp_normal);
            } else if (objects[i].type == PLANE) {
                in_shadow = hitPlane(objects[i], shadow_ray, t_min, t_max, t, temp_normal);
            }
        ...
        if (!in_shadow) {
            intensity = fmaxf(0.0f, normal.dot(light_dir));
        }
