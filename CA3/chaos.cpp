...
int select_random_customer(const vector<int>& orders) {
    ...
    return valid_indices[rand() % valid_indices.size()];
}
...
void* baker_thread(void* arg) {
    ...
    while (true) {
        ...
        int selected_customer = select_random_customer(data->orders);
        ...
        while (data->orders[selected_customer] > 0) {
            pthread_mutex_lock(&ovenLock);
            while (oven_capacity_used == oven_capacity) {
                pthread_cond_wait(&ovenCond, &ovenLock);
            }

            int breads_to_bake = min(data->orders[selected_customer], oven_capacity - oven_capacity_used);
            oven_capacity_used += breads_to_bake;
            data->orders[selected_customer] -= breads_to_bake;

            cout << "Baker " << data->id << " is baking " << breads_to_bake << " breads for "
                 << data->names[selected_customer] << " (" << data->orders[selected_customer] << " remaining).\n";
